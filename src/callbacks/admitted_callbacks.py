...
from dash import callback, Output, Input, State, no_update, html, callback_context as ctx
from dash.exceptions import PreventUpdate
from dash_extensions.javascript import assign

from queries import (
    get_monthly_treatment_comparison_admitted,
    get_top_or_bottom_treatments_admitted,
    get_all_treatment_functions_admitted,
    get_admitted_trend,
    get_icb_boundaries_geojson,
    get_region_bounds,
    get_icb_rtt_for_region_admitted,
    get_icb_imd_scatter_data_admitted,
    get_national_metric_value_admitted,
    get_ccg_imd_geojson,
)
...
# -----ADMITTED PATHWAY CALLBACKS-----


@callback(
    Output("admitted-line-chart", "figure"),
    Input("dummy-trigger-admitted", "children"),
    Input("metric-selector-admitted", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
)
def plot_admitted_national_trend(_, selected_metric, granularity, selected_region_meta):
    ...
    # Fetch data
    df = get_admitted_trend(selected_metric, geo_level=geo_level, nhs_code=nhs_code)
    if df.empty:
    ...
    # Build figure via helper (adds quarterly ticks, FY dividers, optional 3-month avg)
...
@callback(
    Output("admitted-bar-chart", "figure"),
    Output("admitted-selection-warning", "children"),
    Input("admitted-display-chart-btn", "n_clicks"),
    Input("dummy-trigger-admitted", "children"),
    Input("metric-selector-admitted", "value"),
    State("admitted-month-selector", "value"),
    State("admitted-chart-mode", "value"),
    State("admitted-treatment-function-checklist", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
    prevent_initial_call=False,
)
def update_bar_chart_admitted(n_clicks, dummy_trigger, selected_metric, date_val, mode, selected_codes, selected_granularity, selected_region_meta):
    ...
    if mode == "manual":
        ...
        df = get_monthly_treatment_comparison_admitted(year, month, selected_codes, geo_level, nhs_code, selected_metric)
    else:
        order = "desc" if mode == "top" else "asc"
        df = get_top_or_bottom_treatments_admitted(year, month, order, geo_level, nhs_code, 5, selected_metric)
    ...
    total_val = None  # Placeholder for total value
...
# MANUAL SELECTION SECTION FOR ADMITTED PATHWAY-----
@callback(
    Output("admitted-manual-selection-section", "style"),
    Input("admitted-chart-mode", "value"),
)
def toggle_manual_section_admitted(mode):
    if mode == "manual":
    ...
    # ------CALLBACK ADMITTED TREATMENT FUNCTION DROPDOWN-------
@callback(
    Output("admitted-treatment-function-checklist", "data"),
    Input("dummy-trigger-admitted", "children"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
)
def populate_treatment_dropdown_admitted(_, selected_granularity, selected_region_meta):
    ...
    df = get_all_treatment_functions_admitted(geo_level=geo_level, nhs_code=nhs_code)
...
@callback(
    Output("admitted-chart-mode", "options"),
    Output("admitted-chart-mode", "value"),
    Input("metric-selector-admitted", "value"),
    prevent_initial_call=False  
)
def update_chart_mode_options_admitted(selected_metric):
    if not selected_metric:
        raise PreventUpdate
...
@callback(
    Output("admitted-imd-block", "style"),
...
def toggle_imd_block(granularity, pathname):
    ...
    if pathname == "/admitted" and granularity == "Regional":
        return {"display": "block"}
    return {"display": "none"}
...
@callback(
    Output("admitted-icb-rtt-bars", "figure"),
    Input("dummy-trigger-admitted", "children"),
    Input("metric-selector-admitted", "value"),
    Input("admitted-month-selector", "value"),
    Input("admitted-selected-icb", "data"), 
    State("selected-region-meta", "data"),
    State("selected-granularity", "data"),
)
def render_icb_rtt_bars(_, metric, month_value, selected_icb, region_meta, granularity):
    ...
    df = get_icb_rtt_for_region_admitted(year, month, metric, region_code)
...
@callback(
    Output("admitted-selected-icb", "data"),
    Output("admitted-imd-map-map", "center"),
    Output("admitted-imd-map-map", "zoom"),
    Output("admitted-icb-selected", "data"),
    Output("admitted-icb-rtt-bars", "figure", allow_duplicate=True),
    Input("admitted-icb-rtt-bars", "clickData"),           # bar click
    Input("admitted-icb-layer", "clickData"),              # map click
    Input("admitted-clear-icb-selection", "n_clicks"),     # clear
    State("admitted-icb-rtt-bars", "figure"),              # current figure to recolor
    State("metric-selector-admitted", "value"),
    State("admitted-month-selector", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
...
def select_icb(bar_click, map_click, clear_clicks, current_fig, metric, month_value, granularity, region_meta):
    ...
    if triggered == "admitted-icb-layer" and (not map_click or not map_click.get("properties")):
        raise PreventUpdate

    # 1) Clear button → reset selection, clear map highlight, zoom out to region, recolor bars to base
    if triggered == "admitted-clear-icb-selection" and clear_clicks:
    ...
    icb_code = None
    if triggered == "admitted-icb-rtt-bars" and bar_click:
    ...
    elif triggered == "admitted-icb-layer" and map_click:
...
@callback(
    Output("admitted-imd-map", "children"),
    Input("dummy-trigger-admitted", "children"), 
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
    prevent_initial_call=False,
...
@callback(
    Output("admitted-national-imd-block", "style"),
    Input("selected-granularity", "data"),
    Input("url", "pathname"),
    prevent_initial_call=False
)
def toggle_national_imd_block(granularity, pathname):
    if pathname == "/admitted" and granularity == "National":
        return {"display": "block"}
    return {"display": "none"}
...
@callback(
    Output("admitted-national-imd-scatter", "figure"),
    Input("metric-selector-admitted", "value"),
    Input("admitted-month-selector", "value"),
    Input("selected-granularity", "data"),
    prevent_initial_call=False
...
def render_national_imd_scatter(metric, month_value, granularity):
    ...
    df = get_icb_imd_scatter_data_admitted(year, month, metric)
    if df.empty:
        return px.scatter(pd.DataFrame(columns=["x", "y"]))
    ...
    nat_df = get_national_metric_value_admitted(year, month, metric)
     # Scale RTT value if it's a percentage metric
    if "%" in metric:
...
@callback(
    Output("help-drawer-admitted", "opened"),
    Output("help-body-admitted", "children"),
    Input("help-open-btn-admitted", "n_clicks"),
    State("selected-granularity", "data"),
    prevent_initial_call=True,
)
def open_help_admitted(n_clicks, selected_granularity):
    if not n_clicks:
        return no_update, no_update
    
    if selected_granularity == "National":
        content = PAGE_HELP_MIN["admitted_national"]
        body = _render_min_help(content)
    else:
        content = PAGE_HELP_MIN["admitted_regional"]
        body = _render_min_help(content)
    return True, body


@callback(
    Output("metric-help-icon-admitted", "children"),
    Input("metric-selector-admitted", "value"),
)
def update_metric_help(selected_metric):
    ...
    if not key:
        return no_update
    text = METRIC_HELP_TEXT["admitted"].get(key, "")
    return build_help_icon(text)


# --- Regional subtitle ---
@callback(
    Output("admitted-imd-regional-subtitle", "children"),
    Input("metric-selector-admitted", "value"),
    Input("admitted-month-selector", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
...
# --- National subtitle ---
@callback(
    Output("admitted-imd-national-subtitle", "children"),
    Input("metric-selector-admitted", "value"),
    Input("admitted-month-selector", "value"),
    Input("selected-granularity", "data"),
)
...
# --- National Title ---
@callback(
    Output("admitted-title", "children"),
    Input("selected-granularity", "data"),
    Input("selected-region-meta", "data"),
)
def update_admitted_title(granularity, region):
    if granularity == "National":
        title = "Admitted RTT Pathways - National Overview (April 2022- March 2025)"
    else:
        region_name = region["name"]
        title = f"Admitted RTT Pathways - Regional Overview: {region_name} (April 2022- March 2025)"
