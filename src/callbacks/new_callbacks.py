...
from queries import (
    get_monthly_treatment_comparison_new,
    get_top_or_bottom_treatments_new,
    get_all_treatment_functions_new,
    get_new_referrals_trend,
    get_icb_rtt_for_region_new,
    get_icb_boundaries_geojson,
    get_region_bounds,
    get_ccg_imd_geojson,
    get_icb_imd_scatter_data_new,
)
...
import plotly.graph_objects as go
# ----- NEW REFERRALS CALLBACKS --------



@callback(
    Output("new-line-chart", "figure"),
    Input("dummy-trigger-new", "children"),
    Input("metric-selector-new", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
)
def plot_new_referrals_national_trend(_, selected_metric, granularity, selected_region_meta):
    ...
    # Fetch data
    df = get_new_referrals_trend(selected_metric, geo_level=geo_level, nhs_code=nhs_code)
    if df.empty:
    ...
    # Build via shared helper (adds quarterly ticks, FY dividers, and 3-month smoothing)
...
@callback(
    Output("new-bar-chart", "figure"),
    Output("new-selection-warning", "children"),
    Input("new-display-chart-btn", "n_clicks"),
    Input("dummy-trigger-new", "children"),
    Input("metric-selector-new", "value"),
    State("new-month-selector", "value"),
    State("new-chart-mode", "value"),
    State("new-treatment-function-checklist", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
    prevent_initial_call=False,
)
def update_bar_chart_new(n_clicks, dummy_trigger, selected_metric, date_val, mode, selected_codes, selected_granularity, selected_region_meta):

    colors = ["#4E79A7", "#F28E2B", "#E15759", "#76B7B2", "#59A14F"]

    print(f"Updating bar chart for metric: {selected_metric}, mode: {mode}, date: {date_val}, codes: {selected_codes}")
    ...
    if mode == "manual":
        ...
        df = get_monthly_treatment_comparison_new(year, month, selected_codes, geo_level, nhs_code, selected_metric)
    else:
        order = "desc" if mode == "top" else "asc"
        df = get_top_or_bottom_treatments_new(year, month, order, geo_level, nhs_code, 5, selected_metric)
    ...
    for i, row in enumerate(df.itertuples()):
        fig.add_trace(
            go.Bar(
                ...
                hovertemplate=f"<b>{row.treatment_function}</b><br>Total New Referrals Count: {row.value:.2f} <extra></extra>",
            )
        )
    ...
    fig.update_layout(
        ...
        title=title,
        yaxis=dict(
            title="Total New Referrals Count",
...
@callback(
    Output("new-manual-selection-section", "style"),
    Input("new-chart-mode", "value"),
)
def toggle_manual_section_new(mode):
    if mode == "manual":
 ...
 # ------CALLBACK NEW TREATMENT FUNCTION DROPDOWN-------
@callback(
    Output("new-treatment-function-checklist", "data"),
    Input("dummy-trigger-new", "children"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
)
def populate_treatment_dropdown_new(_, selected_granularity, selected_region_meta):
    ...
    df = get_all_treatment_functions_new(geo_level=geo_level, nhs_code=nhs_code)
...
@callback(
    Output("new-chart-mode", "options"),
    Output("new-chart-mode", "value"),
    Input("metric-selector-new", "value"),
    prevent_initial_call=False  
)
def update_chart_mode_options_new(selected_metric):
    if not selected_metric:
        raise PreventUpdate
...
def toggle_imd_block(granularity, pathname):
    ...
    if pathname == "/new-referrals" and granularity == "Regional":
        return {"display": "block"}
    return {"display": "none"}
...
@callback(
    Output("new-icb-rtt-bars", "figure"),
    Input("dummy-trigger-new", "children"),
    Input("metric-selector-new", "value"),
    Input("new-month-selector", "value"),
    Input("new-selected-icb", "data"), 
    State("selected-region-meta", "data"),
    State("selected-granularity", "data"),
)
def render_icb_rtt_bars(_, metric, month_value, selected_icb, region_meta, granularity):
    ...
    df = get_icb_rtt_for_region_new(year, month, metric, region_code)
    ...
    # Metric-aware hover line — ✅ single % symbol
    m = (metric or "").lower()
    if "%" in (metric or ""):
...
@callback(
    Output("new-selected-icb", "data"),
    Output("new-imd-map-map", "center"),
    Output("new-imd-map-map", "zoom"),
    Output("new-icb-selected", "data"),
    Output("new-icb-rtt-bars", "figure", allow_duplicate=True),
    Input("new-icb-rtt-bars", "clickData"),           # bar click
    Input("new-icb-layer", "clickData"),              # map click
    Input("new-clear-icb-selection", "n_clicks"),     # clear
    State("new-icb-rtt-bars", "figure"),              # current figure to recolor
    State("metric-selector-new", "value"),
    State("new-month-selector", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
...
def select_icb(bar_click, map_click, clear_clicks, current_fig, metric, month_value, granularity, region_meta):
    ...
    if triggered == "new-icb-layer" and (not map_click or not map_click.get("properties")):
        raise PreventUpdate

    # 1) Clear button → reset selection, clear map highlight, zoom out to region, recolor bars to base
    if triggered == "new-clear-icb-selection" and clear_clicks:
    ...
    icb_code = None
    if triggered == "new-icb-rtt-bars" and bar_click:
    ...
    elif triggered == "new-icb-layer" and map_click:
...
@callback(
    Output("new-imd-map", "children"),
    Input("dummy-trigger-new", "children"),  # ensure it fires on first load
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
    prevent_initial_call=False,
)
def render_incomplete_imd_map(_, granularity, region_meta):
    ...
    # Data
    ccg_fc = get_ccg_imd_geojson(region_code)         # has 'imd_quintile' (inverted) in properties
    icb_fc = get_icb_boundaries_geojson(region_code)  # has 'icb_label' in properties
    bounds = get_region_bounds(region_code)           # [[S,W],[N,E]]
    ...
    the_map = html.Div(
        [
            dl.Map(
                ...
                minZoom=7,
                maxZoom=10,
                maxBounds=[[49.5, -6.5], [56.2, 2.5]],   # keep view in England-ish box 
...
@callback(
    Output("new-national-imd-block", "style"),
    Input("selected-granularity", "data"),
    Input("url", "pathname"),
    prevent_initial_call=False
)
def toggle_national_imd_block(granularity, pathname):
    if pathname == "/new-referrals" and granularity == "National":
        return {"display": "block"}
    return {"display": "none"}


@callback(
    Output("new-national-imd-scatter", "figure"),
    Input("metric-selector-new", "value"),
    Input("new-month-selector", "value"),
    Input("selected-granularity", "data"),
    prevent_initial_call=False
...
def render_national_imd_scatter(metric, month_value, granularity):
    ...
    df = get_icb_imd_scatter_data_new(year, month, metric)
    if df.empty:
        return px.scatter(pd.DataFrame(columns=["x", "y"]))
...
@callback(
    Output("help-drawer-new", "opened"),
    Output("help-body-new", "children"),
    Input("help-open-btn-new", "n_clicks"),
    State("selected-granularity", "data"),
    prevent_initial_call=True,
)
def open_help_new(n_clicks, selected_granularity):
    if not n_clicks:
        return no_update, no_update
    if selected_granularity == 'National':
        content = PAGE_HELP_MIN["new_national"]
        body = _render_min_help(content)
    else:
        content = PAGE_HELP_MIN["new_regional"]
        body = _render_min_help(content)
    return True, body


@callback(
    Output("metric-help-icon-new", "children"),
    Input("metric-selector-new", "value"),
)
def update_metric_help(selected_metric):
    if not selected_metric:
        return no_update
    key = NEW_REFERRAL_DROPDOWN_MAP.get(selected_metric)
    if not key:
        return no_update
    text = METRIC_HELP_TEXT["new_referrals"].get(key, "")
    return build_help_icon(text)


# --- Regional subtitle ---
@callback(
    Output("new-imd-regional-subtitle", "children"),
    Input("metric-selector-new", "value"),
    Input("new-month-selector", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
...
# --- National subtitle ---
@callback(
    Output("new-imd-national-subtitle", "children"),
    Input("metric-selector-new", "value"),
    Input("new-month-selector", "value"),
    Input("selected-granularity", "data"),
)
...
# --- National Title ---
@callback(
    Output("new-title", "children"),
    Input("selected-granularity", "data"),
    Input("selected-region-meta", "data"),
)
def update_new_title(granularity, region):
    if granularity == "National":
        title = "New RTT Pathways - National Overview (April 2022- March 2025)"
    else:
        region_name = region["name"]
        title = f"New RTT Pathways - Regional Overview: {region_name} (April 2022- March 2025)"
