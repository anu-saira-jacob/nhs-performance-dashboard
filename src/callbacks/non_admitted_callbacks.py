...
from queries import (
    get_monthly_treatment_comparison_non_admitted,
    get_top_or_bottom_treatments_non_admitted,
    get_all_treatment_functions_non_admitted,
    get_non_admitted_trend,
    get_icb_rtt_for_region_non_admitted,
    get_icb_boundaries_geojson,
    get_region_bounds,
    get_ccg_imd_geojson,
    get_icb_imd_scatter_data_non_admitted,
    get_national_metric_value_non_admitted,
)
...
# -----NON-ADMITTED PATHWAY CALLBACKS-----

@callback(
    Output("non-admitted-line-chart", "figure"),
    Input("dummy-trigger-non-admitted", "children"),
    Input("metric-selector-non-admitted", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
)
def plot_non_admitted_national_trend(_, selected_metric, granularity, selected_region_meta):
    ...
    if granularity == "Regional":
        if not selected_region_meta or not selected_region_meta.get("nhs_code"):
    ...
    # Fetch data
    df = get_non_admitted_trend(selected_metric, geo_level=geo_level, nhs_code=nhs_code)
    if df.empty:
    ...
    # Build via helper (quarterly ticks, FY dividers, 3-month avg)
...
@callback(
    Output("non-admitted-bar-chart", "figure"),
    Output("non-admitted-selection-warning", "children"),
    Input("non-admitted-display-chart-btn", "n_clicks"),
    Input("dummy-trigger-non-admitted", "children"),
    Input("metric-selector-non-admitted", "value"),
    State("non-admitted-month-selector", "value"),
    State("non-admitted-chart-mode", "value"),
    State("non-admitted-treatment-function-checklist", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
    prevent_initial_call=False,
)
def update_bar_chart_non_admitted(n_clicks, dummy_trigger, selected_metric, date_val, mode, selected_codes, selected_granularity, selected_region_meta):
    ...
    if mode == "manual":
        ...
        df = get_monthly_treatment_comparison_non_admitted(year, month, selected_codes, geo_level, nhs_code, selected_metric)
    else:
        order = "desc" if mode == "top" else "asc"
        df = get_top_or_bottom_treatments_non_admitted(year, month, order, geo_level, nhs_code, 5, selected_metric)
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=yaxis_title,
            range=[0, max(df["value"].max(), total_val or 0) * 1.2],  # 20% headroom
            gridcolor="#DDE2E6",
        ),
        xaxis=dict(
            title="Treatment Type",
            tickvals=[],  #  Hide cluttered tick labels
...
# MANUAL SELECTION SECTION FOR NON-ADMITTED PATHWAY-----
@callback(
    Output("non-admitted-manual-selection-section", "style"),
    Input("non-admitted-chart-mode", "value"),
)
def toggle_manual_section_non_admitted(mode):
    if mode == "manual":
    ...
    # ------CALLBACK NON ADMITTED TREATMENT FUNCTION DROPDOWN-------
@callback(
    Output("non-admitted-treatment-function-checklist", "data"),
    Input("dummy-trigger-non-admitted", "children"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
)
def populate_treatment_dropdown_non_admitted(_, selected_granularity, selected_region_meta):
    ...
    df = get_all_treatment_functions_non_admitted(geo_level=geo_level, nhs_code=nhs_code)
...
@callback(
    Output("non-admitted-chart-mode", "options"),
    Output("non-admitted-chart-mode", "value"),
    Input("metric-selector-non-admitted", "value"),
    prevent_initial_call=False 
)
def update_chart_mode_options_non_admitted(selected_metric):
    if not selected_metric:
        raise PreventUpdate
...
@callback(
    Output("non-admitted-imd-block", "style"),
    Input("selected-granularity", "data"),
    Input("url", "pathname"),   # ensure it fires when you navigate here
    prevent_initial_call=False
)
def toggle_imd_block(granularity, pathname):
    # Only show on the Incomplete page AND when Regional
    if pathname == "/non-admitted" and granularity == "Regional":
        return {"display": "block"}
    return {"display": "none"}
...
@callback(
    Output("non-admitted-icb-rtt-bars", "figure"),
    Input("dummy-trigger-non-admitted", "children"),
    Input("metric-selector-non-admitted", "value"),
    Input("non-admitted-month-selector", "value"),
    Input("non-admitted-selected-icb", "data"), 
    State("selected-region-meta", "data"),
    State("selected-granularity", "data"),
)
def render_icb_rtt_bars(_, metric, month_value, selected_icb, region_meta, granularity):
    ...
    df = get_icb_rtt_for_region_non_admitted(year, month, metric, region_code)
...
@callback(
    Output("non-admitted-selected-icb", "data"),
    Output("non-admitted-imd-map-map", "center"),
    Output("non-admitted-imd-map-map", "zoom"),
    Output("non-admitted-icb-selected", "data"),
    Output("non-admitted-icb-rtt-bars", "figure", allow_duplicate=True),
    Input("non-admitted-icb-rtt-bars", "clickData"),           # bar click
    Input("non-admitted-icb-layer", "clickData"),              # map click
    Input("non-admitted-clear-icb-selection", "n_clicks"),     # clear
    State("non-admitted-icb-rtt-bars", "figure"),              # current figure to recolor
    State("metric-selector-non-admitted", "value"),
    State("non-admitted-month-selector", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
...
def select_icb(bar_click, map_click, clear_clicks, current_fig, metric, month_value, granularity, region_meta):
    ...
    if triggered == "non-admitted-icb-layer" and (not map_click or not map_click.get("properties")):
        raise PreventUpdate

    # 1) Clear button → reset selection, clear map highlight, zoom out to region, recolor bars to base
    if triggered == "non-admitted-clear-icb-selection" and clear_clicks:
    ...
    icb_code = None
    if triggered == "non-admitted-icb-rtt-bars" and bar_click:
    ...
    elif triggered == "non-admitted-icb-layer" and map_click:
...
@callback(
    Output("non-admitted-imd-map", "children"),
    Input("dummy-trigger-non-admitted", "children"),  
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
    prevent_initial_call=False,
...
def render_incomplete_imd_map(_, granularity, region_meta):
    ...
    the_map = html.Div(
        [
            dl.Map(
                id="non-admitted-imd-map-map",
                bounds=bounds,
                minZoom=7,
                maxZoom=10,
                maxBounds=[[49.5, -6.5], [56.2, 2.5]],   # keep view in England-ish box (adjust if needed)
...
@callback(
    Output("non-admitted-national-imd-block", "style"),
    Input("selected-granularity", "data"),
    Input("url", "pathname"),
    prevent_initial_call=False
)
def toggle_national_imd_block(granularity, pathname):
    if pathname == "/non-admitted" and granularity == "National":
        return {"display": "block"}
    return {"display": "none"}
...
@callback(
    Output("non-admitted-national-imd-scatter", "figure"),
    Input("metric-selector-non-admitted", "value"),
    Input("non-admitted-month-selector", "value"),
    Input("selected-granularity", "data"),
    prevent_initial_call=False
...
def render_national_imd_scatter(metric, month_value, granularity):
    ...
    df = get_icb_imd_scatter_data_non_admitted(year, month, metric)
    if df.empty:
        return px.scatter(pd.DataFrame(columns=["x", "y"]))
    ...
    nat_df = get_national_metric_value_non_admitted(year, month, metric)
     # Scale RTT value if it's a percentage metric
    if "%" in metric:
...
@callback(
    Output("help-drawer-non-admitted", "opened"),
    Output("help-body-non-admitted", "children"),
    Input("help-open-btn-non-admitted", "n_clicks"),
    State("selected-granularity", "data"),
    prevent_initial_call=True,
)
def open_help_non_admitted(n_clicks, selected_granularity):
    if not n_clicks:
        return no_update, no_update
    
    if selected_granularity == "National":
        content = PAGE_HELP_MIN["non_admitted_national"]
        body = _render_min_help(content)
    else:
        content = PAGE_HELP_MIN["non_admitted_regional"]
        body = _render_min_help(content)
    return True, body

@callback(
    Output("metric-help-icon-non-admitted", "children"),
    Input("metric-selector-non-admitted", "value"),
)
def update_metric_help(selected_metric):
    if not selected_metric:
        return no_update
    key = COMPLETED_DROPDOWN_MAP.get(selected_metric)
    if not key:
        return no_update
    text = METRIC_HELP_TEXT["non_admitted"].get(key, "")
...
# --- Regional subtitle ---
@callback(
    Output("non-admitted-imd-regional-subtitle", "children"),
    Input("metric-selector-non-admitted", "value"),
    Input("non-admitted-month-selector", "value"),
    State("selected-granularity", "data"),
    State("selected-region-meta", "data"),
...
# --- National subtitle ---
@callback(
    Output("non-admitted-imd-national-subtitle", "children"),
    Input("metric-selector-non-admitted", "value"),
    Input("non-admitted-month-selector", "value"),
    Input("selected-granularity", "data"),
)
...
# --- National Title ---
@callback(
    Output("non-admitted-title", "children"),
    Input("selected-granularity", "data"),
    Input("selected-region-meta", "data"),
)
def update_non_admitted_title(granularity, region):
    if granularity == "National":
        title = "Non Admitted RTT Pathways - National Overview (April 2022- March 2025)"
    else:
        region_name = region["name"]
        title = f"Non Admitted RTT Pathways - Regional Overview: {region_name} (April 2022- March 2025)"
